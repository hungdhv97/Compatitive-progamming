...
const SERVER = "http://127.0.0.1:5050" // Change to wherver you have the server running
const S3_PREFIX =
...
const NUMBER_OF_FEATURES = 15;

function onDocumentReady() {
  let container = $(".GlowDemo_Container");
  if (container !== undefined) {
    let tabControl = new TabControl(container, ["Tùy chỉnh", "Trộn ảnh"]);
...
class GlowDemoBase {
  ...
  getImageEncodingCacheName(image) {
    const dummy = "_0";
    return image.name + dummy.repeat(NUMBER_OF_FEATURES);
  }
...
class FaceMixingDemo extends GlowDemoBase {
  ...
  constructor(container, options) {
    ...
    let mixingHint = new Hint(
      outputFrame,
      "Chọn đầu vào và kéo thanh để trộn 2 ảnh",
      {
...
class FaceSlidersDemo extends GlowDemoBase {
  ...
  requestManipulation(sliderState) {
    ...
    //console.log(imageEncodingCacheName);
    let cachedImageDataAndVector = this.imageDataAndVectorCache[imageEncodingCacheName];
    //console.log(cachedImageDataAndVector[0], cachedImageDataAndVector[1]);
    if (!cachedImageDataAndVector) {
...
class ImageSelector {
  ...
  constructor(container, onImageSelected, imageRequestsUserUploadCallback, options) {
    ...
    let labelName = (options && options.labelName) || "Đầu vào";
...
class ImageOutput {
  ...
  constructor(container, options) {
    ...
    let label = $(document.createElement("div"));
    let labelName = (options && options.labelName) || "Đầu ra";
    label.html(labelName);
...
function toDataURL(url, callback, fallback = null) {
  var xhr = new XMLHttpRequest();
  xhr.open('get', url + '?x-request=html'); // + '?' +(new Date()).getTime());
  xhr.responseType = 'blob';
  xhr.onload = function () {
    if ((this.status === 404) && (fallback !== null)) {
