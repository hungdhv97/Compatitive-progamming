...
# Căn chỉnh ảnh đầu vào
def get_aligned(img_paths):
	xs = []
	for img_path in img_paths:
		img = Image.open(img_path)
		img = img.convert('RGB') # Nếu ảnh ở dạng RGBA hoặc Grayscale ,....
		img = np.array(img)
...
# Dữ liệu đầu vào. 30000 ảnh sau căn chỉnh có khổi 256x256x3
# x = get_aligned(img_paths)
x = np.load(x.npy)
print('Loaded inputs')

# Mã hóa tất cả đầu vào
def get_z(x):
	bs = 10
...
# Lấy vector thao tác trên các thuộc tính
